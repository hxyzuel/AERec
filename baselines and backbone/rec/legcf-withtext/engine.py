...
if __name__ == "__main__":
    ...
    opt["alias"] = f"{opt['dataset_name']}_assignment_update_frequency:{opt['assignment_update_frequency']}_" \
                   f"num_cluster:{opt['num_clusters']}_seed:{opt['seed']:d}_num_composition_embs:{opt['num_composition_centroid']}" \
                   f"_lr:{opt['lr']:.0e}num_layers:{opt['num_layers']}_num_clusters:{opt['num_clusters']}_" \
                   f"l2_penalty:{opt['l2_penalty_factor']}_anchor_weight:{opt['anchor_weight']}"
