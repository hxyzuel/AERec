...
class FM(BasicRecSys):
    ...
    def __init__(self, opt, data_loader):
        ...
        if opt['use_text']:
            self.mlp = nn.Sequential(
                nn.Linear(opt['text_dims'], opt['mlp_dims']),
                nn.ReLU(),
                nn.BatchNorm1d(opt['mlp_dims']),
                nn.Linear(opt['mlp_dims'], opt['text_dims'])
            )
    ...
    def get_scores_use_text(self, uids, user_embs, iids, item_embs, user_potrait_embed, item_potrait_embed):
        user_embs = torch.concat((user_embs, user_potrait_embed), dim=1)
        item_embs = torch.concat((item_embs, item_potrait_embed), dim=1)
        fm_scores = self.fm(torch.concat((user_embs[:, None, :], item_embs[:, None, :]), dim=1)).squeeze()
        linear_scores = self.linear(torch.concat((uids[:, None], iids[:, None]), dim=1)).squeeze()
        return fm_scores, linear_scores
    ...
    def calculate_training_loss_use_text(self, uids, pos_iids, neg_iids, user_potrait_embed, pos_item_potrait_embed,
        ...
        user_potrait_embed = self.mlp(user_potrait_embed)
        pos_item_potrait_embed = self.mlp(pos_item_potrait_embed)
        neg_item_potrait_embed = self.mlp(neg_item_potrait_embed)

        pos_fm_scores, pos_linear_scores = self.get_scores_use_text(uids, user_embs, pos_iids, pos_embs,
                                                                    user_potrait_embed, pos_item_potrait_embed)
        pos_scores = pos_fm_scores + pos_linear_scores
        assert not torch.isnan(pos_fm_scores).any()
        assert not torch.isnan(pos_linear_scores).any()
        neg_fm_scores, neg_linear_scores = self.get_scores_use_text(uids, user_embs, neg_iids, neg_embs,
                                                                    user_potrait_embed, neg_item_potrait_embed)
        assert not torch.isnan(neg_fm_scores).any()
        assert not torch.isnan(neg_linear_scores).any()
        neg_scores = neg_fm_scores + neg_linear_scores
        bpr_loss = BPR_loss(pos_scores, neg_scores)
    ...
    def forward(self, uids, iids, user_potrait_tensor, item_potrait_tensor, use_text):
        ...
        if use_text:
            user_potrait_embed = user_potrait_tensor[uids]
            item_potrait_embed = item_potrait_tensor[iids]
            user_potrait_embed = self.mlp(user_potrait_embed)
            item_potrait_embed = self.mlp(item_potrait_embed)
            fm_scores, linear_scores = self.get_scores_use_text(uids, user_embs, iids, item_embs, user_potrait_embed,
                                                                item_potrait_embed)
        else:
