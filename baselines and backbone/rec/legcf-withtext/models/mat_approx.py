...
class MatApprox(nn.Module):
    def __init__(self, opt: dict, data_loader: DataLoader):
        ...
        self.anchor_weight = opt["anchor_weight"]
        ...
        if not opt["use_metis_init"]:
            ...
            # 权重分配矩阵，形状N*c，N为item数量+user数量，c为meta-embedding的数量
            self.centroid_assignment = self.init_centroid_assignment_using_metis(
                (sum(self.field_dims), self.num_clusters),
                non_zero_row_count=self.num_composition_centroid, selected_assignment_weight=self.anchor_weight).to(opt["device_id"])
            # meta-embedding矩阵，形状c*d，c为meta-embedding的数量，d为item和user正式向量的维度
            self.centroid_embs = torch.zeros((self.num_clusters, self.latent_dim))
            # init using normal
            nn.init.normal_(self.centroid_embs)
    ...
    def init_centroid_assignment_using_metis(self, shape: tuple, non_zero_row_count: int = 2,
                                             selected_assignment_weight: float = .9):
        ...
        for idx in range(self.num_clusters):
            ...
            if self.num_composition_centroid == 1:
                ...
                for _entity in selected_entities:
                    ...
                    assignment_mat[_entity, other_assignments] = (1 - selected_assignment_weight) / (
    ...
    def get_full_embs(self):
        ...
        user和item正式嵌入矩阵，形状N*d，N为item数量+user数量，d为item和user正式向量的维度
        """
